...
# Latest ECS-optimized Amazon Linux 2023 AMI, published as an SSM parameter.
...
# Bridge networking uses dynamic host ports (32768-65535) for ALB targets.
resource "aws_security_group" "app" {
  name_prefix = "${var.project_name}-app-"
  vpc_id      = aws_vpc.main.id

  ingress {
    description     = "Ephemeral container ports from ALB"
    from_port       = 32768
    to_port         = 65535
    protocol        = "tcp"
...
# --------------------------------------------------------------------------
# IAM: task execution role + EC2 instance role
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# ECS cluster + EC2 capacity (Launch Template + Auto Scaling group)
# --------------------------------------------------------------------------
...
resource "aws_launch_template" "ecs" {
  ...
  # Register the instance with this ECS cluster.
  ...
  tag_specifications {
    ...
    tags          = merge(local.common_tags, { Name = "${var.project_name}-ecs-instance" })
  }
}
...
# --------------------------------------------------------------------------
# Task definitions (EC2 launch type, bridge networking, dynamic host ports)
# --------------------------------------------------------------------------
