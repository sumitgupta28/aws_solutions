...
variable "instance_type" {
  ...
  validation {
    ...
    error_message = "Use t3.micro, t3.small, or t3.medium for this demo (t3.small recommended)."
  }
}

variable "desired_capacity" {
  description = "Number of EC2 container instances in the Auto Scaling group."
  type        = number
  default     = 2
}
...
variable "backend_memory" {
  description = "Hard memory limit (MiB) for the backend container."
  type        = number
  default     = 768
}

variable "frontend_memory" {
  description = "Hard memory limit (MiB) for the frontend container."
  type        = number
  default     = 256
}
